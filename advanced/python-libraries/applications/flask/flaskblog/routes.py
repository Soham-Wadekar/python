...
from flaskblog.forms import RegistrationForm, LoginForm, UpdateAccount, PostForm, RequestResetForm, ResetPasswordForm
...
def new_post():
    form = PostForm()
    if form.validate_on_submit():
        post = Post(title=form.title.data, text=form.content.data, author=current_user)
        db.session.add(post)
        db.session.commit()
